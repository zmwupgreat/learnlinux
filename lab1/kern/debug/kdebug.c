...
uint32_t ebp1=read_ebp();     // (1) call read_ebp() to get the value of ebp. the type is (uint32_t);
uint32_t eip1=read_eip();      //(2) call read_eip() to get the value of eip. the type is (uint32_t);
     int i,j;
	 for(i=0;i<STACKFRAME_DEPTH&&ebp!=0;i++)//(3) from 0 .. STACKFRAME_DEPTH
     {   cprintf("ebp1:%08x,eip1:%08x,args:",ebp1,eip1);  //(3.1) printf value of ebp, eip
      	   for(j=0;j<4;j++)//(3.2) (uint32_t)calling arguments [0..4] = the contents in address (unit32_t)ebp +2 [0..4]
      {
	  			cprintf("%08x",*((unit32_t)*)ebp1+2+j);
	  }

	    cprintf("\n");
      print_debuginfo(eip1-1);  //  (3.4) call print_debuginfo(eip-1) to print the C calling function name and line number, etc.
        // (3.5) popup a calling stackframe
            eip1=*((uint32_t*)ebp1+1);  //  NOTICE: the calling funciton's return addr eip  = ss:[ebp+4]
            ebp1=*((uint32_t*)ebp1);    //         the calling funciton's ebp = ss:[ebp]
     } 
}
