...
alloc_proc(void) {
     ...
     */
	proc->state=PROC_UNINIT;//设置为未初始化
	proc->pid=-1;//未初始化
	proc->runs=0;//时间片
	proc->kstack=0;//内存栈地址
	...
	proc->mm=NULL;//虚存
	memset(&(proc->mm),0,sizeof(struct context));//上下文
	proc->tf=NULL;//中断帧指针
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
   ...
   proc=alloc_proc();
	if(proc==NULL) //    1. call alloc_proc to allocate a proc_struct
{
	goto fork_out;
}
proc->parent=current;
   skstack=setup_kstack(proc); 
	if(skstack!=-E_NO_MEM)
{
	goto bad_fork_cleanup_proc;
}		//    2. call setup_kstack to allocate a kernel stack for child process
   cmm=copy_mm(clone_flags,proc);
	if(cmm!=0)//    3. call copy_mm to dup OR share mm according clone_flag
	{
		goto bad_fork_cleanup_proc;
	}
